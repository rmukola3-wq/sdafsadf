...
local function alphaFromSpeed(speed, dt)
	-- speed: "насколько быстро догоняем" (больше = резче)
...
-- Импортируем новую систему отдачи из ВНЕДРЕНИЕ
local RecoilSystem = require(Mods:WaitForChild("RecoilSystem"))

-- Загрузка профилей из папки (RecoilProfiles - это Folder, а не ModuleScript)
...
-- возвращает true если сейчас идёт анимация которую нельзя прерывать
...
local wallCF		= CFrame.new() -- wall pushback offset (отжим ствола от стены)
...
-- НОВАЯ СИСТЕМА ОТДАЧИ из ВНЕДРЕНИЕ
-- Stateful spring-based recoil с поддержкой паттернов
local gunRecoilState = RecoilSystem.new()

-- Старый RecoilSpring оставляем для совместимости (будет удалён позже)
local RecoilSpring = SpringMod.new(Vector3.new())
RecoilSpring.d = 0.18
RecoilSpring.s = 16

local cameraspring = SpringMod.new(Vector3.new())
cameraspring.d	= 0.38  -- было .5 — меньше демпфирования = больше "живости"
cameraspring.s	= 14    -- было 20 — камера дольше восстанавливается

local SwaySpring = SpringMod.new(Vector3.new())
SwaySpring.d = .25
SwaySpring.s = 20

-- VMKick: viewmodel positional kick on shot
local VMKickPos = SpringMod.new(Vector3.new())
VMKickPos.d = 0.92
VMKickPos.s = 60

-- VMKick: viewmodel rotational kick on shot
...
-- Weapon Lag / Inertia Sway (ствол запаздывает за движением мыши)
local LagSpring = SpringMod.new(Vector3.new())
LagSpring.d = 0.92  -- damping (увеличено для более плавного движения)
LagSpring.s = 24    -- speed (увеличено для быстрого возвращения)

-- Weapon Lag tuning (hip vs ads)
local LAG_ROT_HIP = 0.0040      -- угловое отставание в hipfire (УВЕЛИЧЕНО для большего запаздывания)
local LAG_POS_HIP = 0.0013     -- позиционное отставание в hipfire (УВЕЛИЧЕНО для большего запаздывания)

local LAG_ROT_ADS = 0.00045     -- ADS должен быть маленьким, чтобы мушку не мотало
local LAG_POS_ADS = 0.00012

local LAG_MAX_HIP = math.rad(8) -- ограничение угла (УВЕЛИЧЕНО)
...
function handleAction(actionName, inputState, inputObject)
	...
	if actionName == "Run" and inputState == Enum.UserInputState.Begin and not script.Parent:GetAttribute("Injured") then
		...
		-- если уже движемся — сразу включаем спринт
		if running then
			Stand()
			Stances = 0
			Virar = 0
...
function ADS(aimming)
	if WeaponData and WeaponInHand then
		...
		-- НОВАЯ СИСТЕМА ОТДАЧИ из ВНЕДРЕНИЕ
		-- Сбрасываем spray pattern при выходе из прицела
...
function setup(Tool)
	-- анти-спам токен: запоминаем свой номер
	local myToken = EquipToken

	-- санитарный килл: уничтожаем все старые Viewmodel в камере
	...
	-- каждый вызов получает свою версию
	SetupVersion += 1
	...
	if char and char:WaitForChild("Humanoid").Health > 0 and Tool ~= nil then
		...
		-- если пока мы делали WaitForChild/Clone пришёл новый setup() — выходим
		...
		-- Ставим AnimPart сразу в правильную позицию чтобы руки не падали
		-- в мир в первый кадр до того как RenderStepped успел отработать
...
function unset()
	...
	-- Если идёт перезарядка/передёргивание/проверка магазина — ждём завершения
	-- (но не дольше 3 сек на случай зависания флага)
	...
	-- Если за время ожидания уже запустился новый unset — выходим
	if UnsetVersion ~= myUnset then return end
	...
	-- UI/камера сбрасываются асинхронно в task.spawn ниже (после анимации)
	...
	if WeaponInHand then
		...
		-- 3) ТЕПЕРЬ синхронно уничтожаем oldVM — до task.spawn и до setup()
		-- Destroy() убирает из Camera немедленно в этом же кадре
		if oldVM then
			Debris:AddItem(oldVM, 0)        -- страховка если Destroy бросит ошибку
			pcall(function() oldVM:Destroy() end)
			oldVM = nil
		end

		-- 3) фон: только возвращаем UI/камеру после задержки
		task.spawn(function()
			task.wait(0.15)

			-- возвращаем UI/камеру ТОЛЬКО если за это время не экипнули другое оружие
		...
		-- 4) обнуляем переменные (oldVM уже сохранён, ViewModel:Destroy() НЕ вызываем здесь)
		ViewModel 		= nil
		WeaponInHand	= nil
		WeaponTool		= nil
		LArm 			= nil
		...
		BipodCF = CFrame.new()
		wallCF = CFrame.new() -- сбрасываем wall push при смене оружия
		if gameRules.ReplicatedLaser then
			Evt.SVLaser:FireServer(nil,2,nil,false,WeaponTool)
		end
		
		-- НОВАЯ СИСТЕМА ОТДАЧИ из ВНЕДРЕНИЕ
		-- Сбрасываем состояние отдачи при смене оружия
...
function renderGunRecoil()
	-- НОВАЯ СИСТЕМА ОТДАЧИ из ВНЕДРЕНИЕ
	-- Функция больше не нужна - обновление происходит в RenderStepped
	-- НЕЛЬЗЯ использовать task.wait() в RenderStepped!
end

function Recoil()
	-- НОВАЯ СИСТЕМА ОТДАЧИ из ВНЕДРЕНИЕ
	-- Используем stateful spring-based recoil вместо math.random
	
	-- Получаем профиль отдачи для текущего оружия
	local name = WeaponData.RecoilProfile or "Rifle556"
	local profile = GetRecoilProfile(name) or GetRecoilProfile("Rifle556")
	print("[Recoil] Using:", name, "OK=", profile ~= nil, "inst=", profile and profile.instantKickRatio)
	if not profile then return end
	
	-- Устанавливаем профиль если изменился
	if gunRecoilState.currentProfile ~= profile then
		gunRecoilState:SetProfile(profile)
	end
	
	-- ADS multipliers для стабильного прицеливания
	local adsViewKickScale = (profile and profile.adsViewKickScale) or 0.35   -- насколько меньше VMKick в ADS
	local adsRecoilCFScale = (profile and profile.adsRecoilCFScale) or 0.45   -- насколько меньше RecoilSystem recoilcf в ADS
	local adsSwayScale     = (profile and profile.adsSwayScale) or 0.22       -- sway меньше в ADS
	local adsLagScale      = (profile and profile.adsLagScale) or 0.15        -- lag меньше в ADS
	local adsCamRecoilPower = (profile and profile.adsCamRecoilPower) or 0.18 -- маленькая отдача в камеру ТОЛЬКО ADS
	
	-- Вычисляем множитель силы отдачи
	local mult = 1.0
	
	-- ADS множитель
	if aimming then
		mult = mult * (profile.aimMultiplier or 0.3)
	end
	
	-- Crouch множитель
	if Crouched then
		mult = mult * (profile.crouchMultiplier or 0.65)
	end
	
	-- Bipod множитель (очень сильно снижает отдачу)
	if BipodActive then
		mult = mult * 0.3
	end
	...
	if charspeed > 0.1 then
		mult = mult * (profile.moveMultiplier or 1.2)
	end
	
	-- Усиливаем ОТДАЧУ СТВОЛА/ВЬЮМОДЕЛИ примерно в 5.5 раза
	mult = mult * (RecoilPower or 1) * 5.5
	
	-- Hipfire only recoil scale (4-6x softer)
	local hipRecoilScale = (profile and profile.hipRecoilScale) or 0.20  -- 0.25 = ~4x softer, 0.17 = ~6x softer
	if not aimming then
		mult = mult * hipRecoilScale
	end
	
	-- Уменьшаем recoilcf (RecoilSystem) только в ADS
	if aimming then
		mult = mult * adsRecoilCFScale
	end
	
	-- КИКАЕМ новую систему отдачи
	gunRecoilState:Kick(profile, mult)
	
	-- Проверка: print hipfire scales
	if not aimming then
		print("[Recoil] hip scales:", hipRecoilScale, hipVMKickScale)
	end
	
	-- Камерная отдача ТОЛЬКО в ADS (делает прицеливание естественным)
	if aimming then
	...
	-- VMKick: viewmodel positional kick
	...
	-- Hipfire only VMKick scale (4-6x softer)
	local hipVMKickScale = (profile and profile.hipVMKickScale) or 0.20
	if not aimming then
		kick = kick * hipVMKickScale
	end
	
	-- Уменьшаем viewmodel "удар" только в ADS (чтобы мушку не сносило)
	local viewKickScale = aimming and adsViewKickScale or 1
	local kickVM = kick * viewKickScale

	-- Усиливаем VMKick множителем vm = 0.75 (уменьшено в 2 раза от 1.5)
	local vm = 0.75
	
	-- Уменьшаем удар ствола в 2.9 раза (~на 66%)
	local hitScale = 0.345 -- weaker punch (~2.9x softer)
	
	-- Уменьшаем высоту подброса вверх в 1.2 раза
	local vertScale = 0.83 -- 1/1.2, ниже подброс вверх
	...
	VMKickRot:accelerate(Vector3.new(
		math.rad(3.4) * kickVM * vm * hitScale * vertScale, -- pitch меньше
		math.rad(RAND(-2.2, 2.2, 0.5)) * kickVM * vm * hitScale, -- yaw не трогаем
		math.rad(RAND(-1.5, 2.2, 0.5)) * kickVM * vm * hitScale  -- roll не трогаем
	))
end
...
function L_150_.Update()
	...
	-- delta method: apply only the change since last frame to avoid accumulation
...
function UnEquipAnim()
	-- безопасно: даже если в оружии нет UnEquipAnim
	pcall(function()
		if AnimData and AnimData.UnEquipAnim then
			AnimData.UnEquipAnim({
				RArmWeld,
				LArmWeld,
				GunWeld,
				WeaponInHand,
...
----------//Gun System\\----------
-- Один воркер для свапа — никогда не создаётся больше одного task.spawn
local function RequestEquip(tool)
	PendingTool = tool
	EquipToken += 1

	if SwitchWorkerRunning then
		return
	end
...
Run.RenderStepped:Connect(function(step)
	...
	if ViewModel and LArm and RArm and WeaponInHand then --Check if the weapon and arms are loaded
		...
		-- Weapon Lag / Inertia Sway (hip vs ads, stable across FPS)
		...
		-- target для пружины (угловое отставание)
		...
		-- Wall pushback: если ствол близко к стене — отжимаем оружие назад
		if WeaponInHand and WeaponInHand:FindFirstChild("Handle") and WeaponInHand.Handle:FindFirstChild("Muzzle") then
			local wallCheckDist = 2        -- studs, расстояние проверки
			local wallMaxPush   = 0.6      -- макс откат по Z (стадс)
			local muzzleAtt    = WeaponInHand.Handle.Muzzle
			local muzzleOrigin = muzzleAtt.WorldPosition
			local muzzleFwd    = muzzleAtt.WorldCFrame.LookVector

			local wallParams = RaycastParams.new()
			wallParams.FilterType = Enum.RaycastFilterType.Exclude
			wallParams.FilterDescendantsInstances = Ignore_Model

			local wallResult = workspace:Raycast(muzzleOrigin, muzzleFwd * wallCheckDist, wallParams)

			local wallTargetZ = 0
			if wallResult then
				local dist = (wallResult.Position - muzzleOrigin).Magnitude
				local pushAmount = math.clamp(wallMaxPush - (dist / wallCheckDist) * wallMaxPush, 0, wallMaxPush)
				if aimming then pushAmount = pushAmount * 0.35 end
				wallTargetZ = pushAmount
			end

			local curZ = wallCF.Z
			wallCF = CFrame.new(0, 0, curZ + (wallTargetZ - curZ) * alphaFromSpeed(14, step))
		else
			wallCF = CFrame.new(0, 0, wallCF.Z * (1 - alphaFromSpeed(14, step)))
		end

		AnimPart.CFrame = cam.CFrame * NearZ * BipodCF * maincf * lagCF * gunbobcf * aimcf * recoilcf * wallCF * vmKickCF
