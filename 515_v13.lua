...
local SwitchToken   = 0
local SetupVersion  = 0  -- каждый setup() получает уникальный номер
...
function setup(Tool)
	...
	-- если пока setup выполнялся пришёл новый вызов — старый тихо выходит
	SetupVersion += 1
	...
	if char and char:WaitForChild("Humanoid").Health > 0 and Tool ~= nil then
		...
		-- не парентим VM в камеру, уничтожаем недособранный VM
		if myVersion ~= SetupVersion then
	...
	if Tool:IsA('Tool') and Humanoid.Health > 0 and Tool:FindFirstChild("ACS_Settings") ~= nil and (require(Tool.ACS_Settings).Type == 'Gun' or require(Tool.ACS_Settings).Type == 'Melee' or require(Tool.ACS_Settings).Type == 'Grenade') then
		...
		if L_370_ then
			...
			if not ToolEquip then
				...
				task.spawn(function()
					unset()
					-- если за время unset() игрок ещё раз переключил — выходим
