...
-- SwaySpring остаётся для покачивания при свее
local SwaySpring = SpringMod.new(Vector3.new())
SwaySpring.d = .25
SwaySpring.s = 20

-- ============================================================
-- RECOIL MODULE (встроен — порт из хорошего ACS)
-- Физика: пружинный интегратор с damping/stiffness = 12.75/56.25
-- ============================================================
local RecoilModule = {}
RecoilModule.__index = RecoilModule
function RecoilModule.new()
	local s = setmetatable({}, RecoilModule)
	s.pitch=0; s.yaw=0; s.roll=0
	s.pitchVel=0; s.yawVel=0; s.rollVel=0
	s.posX=0; s.posY=0; s.posZ=0
	s.velX=0; s.velY=0; s.velZ=0
	s.shotsFired=0; s.sprayAmount=0
	s.lastShotTime=0; s.patternIndex=0; s.bounceDirection=1
	return s
end
function RecoilModule:Kick(profile, mult)
	local now = os.clock()
	local dt  = now - self.lastShotTime
	local p   = profile or {}
	local m   = (mult or 1) * 0.5
	if (p.sprayDecayTime or 0.4) < dt then
		self.sprayAmount = math.max(0, self.sprayAmount - dt * 3.5)
		if self.sprayAmount < 0.05 then
			self.sprayAmount = 0; self.shotsFired = 0; self.patternIndex = 0
		end
	end
	self.lastShotTime = now
	self.shotsFired   = self.shotsFired + 1
	self.sprayAmount  = self.sprayAmount + (p.sprayIncrease or 0.08) / (1 + self.shotsFired * 0.035)
	local speed = (p.patternSpeed or 25) * (math.min(dt, 0.1) * 2 + 0.8)
	local kickUp   = (p.kickStrength or 0.08) * (1 + (math.random()-0.5)*(p.kickRandomness or 0.3)) * (1 + self.sprayAmount)
	local kickSide = (p.horizontalStrength or 0.04) * (math.random()*2-1) * (1 + (math.random()-0.5)*(p.horizontalRandomness or 0.5))
	if p.usePattern and p.pattern and #p.pattern > 0 then
		self.patternIndex = math.min(self.patternIndex+1, #p.pattern)
		local pt = p.pattern[self.patternIndex]
		local sc = (p.patternScale or 1) * (1+(math.random()-0.5)*(p.patternRandomness or 0.1)*0.8)
		kickUp = pt.y*sc; kickSide = pt.x*sc
	end
	if p.horizontalBounce then
		kickSide = kickSide * self.bounceDirection * (p.bounceAmount or 0.7)
		self.bounceDirection = -self.bounceDirection
	end
	kickUp   = kickUp   * m * speed
	kickSide = kickSide * m * speed
	self.pitchVel = self.pitchVel + kickUp
	self.yawVel   = self.yawVel   + kickSide
	self.rollVel  = self.rollVel  + ((p.rollStrength or 0.02)*(math.random()*2-1))*(m*speed)*0.7 - kickSide*0.25
	self.velZ     = self.velZ     - (p.gunKickback or 0.06)*speed*1.1
	self.velY     = self.velY     + (p.gunRise     or 0.04)*speed*0.75
	self.velX     = self.velX     + kickSide*0.25
end
function RecoilModule:Update(dt)
	local t = math.min(dt, 0.1)
	local function sp(pos, vel)
		local nv = vel + (-56.25*pos - 12.75*vel)*t
		local np = pos + nv*t
		if math.abs(np)<0.0008 and math.abs(nv)<0.008 then return np*0.5, nv*0.5 end
		return np, nv
	end
	self.pitch,  self.pitchVel = sp(self.pitch,  self.pitchVel)
	self.yaw,    self.yawVel   = sp(self.yaw,    self.yawVel)
	self.roll,   self.rollVel  = sp(self.roll,   self.rollVel)
	self.posX,   self.velX     = sp(self.posX,   self.velX)
	self.posY,   self.velY     = sp(self.posY,   self.velY)
	self.posZ,   self.velZ     = sp(self.posZ,   self.velZ)
	self.sprayAmount = math.max(0, self.sprayAmount - t*2.2)
	self.pitch = math.clamp(self.pitch, -0.3,  0.8)
	self.yaw   = math.clamp(self.yaw,   -0.5,  0.5)
	self.roll  = math.clamp(self.roll,  -0.35, 0.35)
	return CFrame.new(self.posX*0.25, self.posY*0.35, self.posZ*0.4)
		* CFrame.Angles(self.pitch, self.yaw, self.roll)
end
function RecoilModule:Reset()
	self.pitch=0; self.yaw=0; self.roll=0
	self.pitchVel=0; self.yawVel=0; self.rollVel=0
	self.posX=0; self.posY=0; self.posZ=0
	self.velX=0; self.velY=0; self.velZ=0
	self.shotsFired=0; self.sprayAmount=0; self.patternIndex=0
end

-- ============================================================
-- RECOIL CAMERA (встроен — порт из хорошего ACS)
-- Применяет импульс к камере и сам сглаживает его в RenderStepped
-- ============================================================
local RecoilCamera = {}
do
	local _pending = CFrame.new()
	function RecoilCamera.Recoil(pitchDeg, yawDeg, rollDeg)
		local sign = (math.random() > 0.5) and 1 or -1
		_pending = _pending * CFrame.Angles(
			math.rad(pitchDeg),
			math.rad(sign * yawDeg) * 1.15,
			math.rad(sign * rollDeg) * 2.35
		)
	end
	Run.RenderStepped:Connect(function(dt)
		cam.CFrame = cam.CFrame * _pending
		_pending = _pending:Lerp(CFrame.new(), math.min(dt * 28, 1))
	end)
end

-- Инстанс состояния отдачи оружия (создаётся один раз, Reset при смене)
local gunRecoilState = RecoilModule.new()
...
function renderCam()
	-- Камерная отдача обрабатывается RecoilCamera (свой RenderStepped выше)
end

function renderGunRecoil()
	-- recoilcf обновляется в RenderStepped через gunRecoilState:Update(step)
end
...
function Recoil()
	...
	-- множитель прицеливания: при ADS отдача слабее
	local aimM  = aimming and (1 / math.max(ARR, 0.1)) or 1
	-- множитель биподы
	local bipoM = BipodActive and 0.25 or 1
	local mult  = aimM * bipoM

	-- ---- профиль для визуальной отдачи ствола ----
	local gUp   = math.random(WeaponData.gunRecoil.gunRecoilUp[1],   WeaponData.gunRecoil.gunRecoilUp[2])   / 10 * ModTable.gunRecoilMod.RecoilUp
	local gSide = math.random(WeaponData.gunRecoil.gunRecoilLeft[1], WeaponData.gunRecoil.gunRecoilRight[2]) / 10 * ModTable.gunRecoilMod.RecoilLeft
	local gTilt = math.random(WeaponData.gunRecoil.gunRecoilTilt[1], WeaponData.gunRecoil.gunRecoilTilt[2])  / 10 * ModTable.gunRecoilMod.RecoilTilt

	local gunProfile = {
		kickStrength       = gUp   * RecoilPower * 0.015,
		horizontalStrength = gSide * RecoilPower * 0.008,
		rollStrength       = gTilt * 0.005,
		gunKickback        = gUp   * RecoilPower * 0.012,
		gunRise            = gUp   * RecoilPower * 0.010,
		kickRandomness     = 0.25,
		horizontalRandomness = 0.5,
		sprayIncrease      = 0.06,
		sprayDecayTime     = 0.35,
		patternSpeed       = 22,
	}
	gunRecoilState:Kick(gunProfile, mult)

	-- ---- отдача камеры через RecoilCamera ----
	local camUp   = math.random(WeaponData.camRecoil.camRecoilUp[1],    WeaponData.camRecoil.camRecoilUp[2])    / 2 * ModTable.camRecoilMod.RecoilUp    * mult
	local camSide = math.random(WeaponData.camRecoil.camRecoilRight[1], WeaponData.camRecoil.camRecoilRight[2]) / 2 * ModTable.camRecoilMod.RecoilRight  * mult
	local camTilt = math.random(WeaponData.camRecoil.camRecoilTilt[1],  WeaponData.camRecoil.camRecoilTilt[2])  / 2 * ModTable.camRecoilMod.RecoilTilt   * mult

	RecoilCamera.Recoil(
		camUp   * RAND(0.8, 1.2, 0.1),
		camSide * RAND(-1,  1,   0.1),
		camTilt * RAND(-1,  1,   0.1)
	)
end
	...
	if gameRules.EnableFallDamage then
		if state == Enum.HumanoidStateType.Freefall and not falling then
			...
			if damage > 5 and peak > 20 then
				...
				RecoilCamera.Recoil(-damage/20, 0, math.random(-damage, damage)/5)
...
Run.RenderStepped:Connect(function(step)
	...
	if ViewModel and LArm and RArm and WeaponInHand then --Check if the weapon and arms are loaded
		...
		do
			...
			local direction = cam.CFrame.LookVector * GunSize
			local result = workspace:Raycast(origin, direction, params)

			if result then
				local dist = (result.Position - origin).Magnitude
				if dist < GunSize then
					local push = ((dist / GunSize) - 1) * -2
					PushbackCF = CFrame.new(0,0,push)
					IsPushingBack = true
				end
			end
		end

		AnimPart.CFrame = cam.CFrame * NearZ * PushbackCF * BipodCF * maincf * gunbobcf * aimcf
