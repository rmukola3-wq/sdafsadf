...
LagSpring.d = 0.85  -- damping
LagSpring.s = 18    -- speed

local LAG_ROT      = 0.0022        -- сила углового лага
local LAG_POS      = 0.0009        -- сила позиционного сдвига
local LAG_ADS_MULT = 0.35          -- в прицеле меньше
local LAG_MAX      = math.rad(6)   -- ограничение угла
...
function renderGunRecoil()
	-- RecoilSpring.p already in radians (math.rad used in accelerate calls)
	-- do NOT apply here; recoilcf is lerped in RenderStepped using spring values directly
end
...
function Recoil()
	...
	-- VMKick impulse: visible gun kick on shot
	local kick = 1
	if aimming then kick = 0.65 end
	if BipodActive then kick = kick * 0.4 end

	-- hipfire multiplier: reduce recoil when not aiming
	local hipMult = (not aimming) and (WeaponData.HipRecoilMult or 0.65) or 1
	kick = kick * hipMult

	VMKickPos:accelerate(Vector3.new(0, 0.99 * kick, -1.20 * kick))
	VMKickRot:accelerate(Vector3.new(
		math.rad(2.6) * kick,
		math.rad(RAND(-1.8, 1.8, 0.5)) * kick,
		math.rad(RAND(-1.2, 1.8, 0.5)) * kick
		))
end
...
local function RequestEquip(tool)
	PendingTool = tool
	EquipToken += 1
	-- print("RequestEquip", tool.Name)

	if SwitchWorkerRunning then
		return  -- воркер уже крутится, он возьмёт свежий PendingTool
	end
...
char.ChildAdded:connect(function(Tool)
	if Tool:IsA('Tool') and Humanoid.Health > 0 and Tool:FindFirstChild("ACS_Settings") ~= nil and (require(Tool.ACS_Settings).Type == 'Gun' or require(Tool.ACS_Settings).Type == 'Melee' or require(Tool.ACS_Settings).Type == 'Grenade') then
		...
		if L_370_ then
			L_199_ = Tool
			-- спам того же оружия — игнорируем
...
char.ChildRemoved:Connect(function(Tool)
	-- даём 0.05с шанс ChildAdded/setup обновить WeaponTool на новое оружие.
	-- если свап — WeaponTool уже изменится и мы не трогаем новый сетап.
	-- если реальный unequip — WeaponTool останется тем же и unset() сработает.
...
Run.RenderStepped:Connect(function(step)
	...
	if ViewModel and LArm and RArm and WeaponInHand then --Check if the weapon and arms are loaded
		...
		-- Weapon Lag: ствол "запаздывает" за движением мыши
		local lagMult = aimming and LAG_ADS_MULT or 1
		LagSpring.t = Vector3.new(
			math.clamp(-mouseDelta.Y * LAG_ROT * lagMult, -LAG_MAX, LAG_MAX),
			math.clamp(-mouseDelta.X * LAG_ROT * lagMult, -LAG_MAX, LAG_MAX),
			0
		)
		local lp = LagSpring.p
		local lagCF =
			CFrame.new(
				math.clamp(-mouseDelta.X * LAG_POS * lagMult, -0.08, 0.08),
				math.clamp(-mouseDelta.Y * LAG_POS * lagMult, -0.06, 0.06),
				0
			)
		...
		AnimPart.CFrame = cam.CFrame * NearZ * BipodCF * maincf * lagCF * gunbobcf * aimcf * recoilcf * vmKickCF
		...
		recoilcf = recoilcf:Lerp(CFrame.new() * CFrame.Angles(RecoilSpring.p.X, RecoilSpring.p.Y, RecoilSpring.p.Z), 0.2)
